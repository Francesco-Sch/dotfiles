...
*** Marginalia
Adds *rich metadata* to the side of the completion list. This can be file modes, docstrings or command keybindings.
...
These commands replace regular commands like =switch-to-buffer= and add additional functionalities like live previewing and ripgrep search.
